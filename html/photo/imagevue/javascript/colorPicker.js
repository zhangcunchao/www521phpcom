...
Control.ColorPicker.prototype = {
  initialize : function(field, options) {
    ...
    this.options = Object.extend({
       IMAGE_BASE : "images/admin/colorpicker/"
    }, options || {});
    ...
    if (!Control.ColorPicker.CONTROL) {
      Control.ColorPicker.CONTROL = {};
      if (!$("colorpicker")) {
        var control = new Element('div', {id: 'colorpicker'});
        control.innerHTML = 
          ...
          '<span id="colorpicker-value">#<input type="text" onclick="this.select()" id="colorpicker-value-input" name="colorpicker-value" value=""></input></span><button id="colorpicker-okbutton">OK</button>' +
          '<br />' +
          '<div><button onclick="Control.ColorPicker.activeColorPicker.field.setStyle({backgroundColor: \'#FFF\', color: \'#000\'}).value = \'foreground_color\'; Control.ColorPicker.activeColorPicker.close();">foreground color</button></div>' +
          '<div><button onclick="Control.ColorPicker.activeColorPicker.field.setStyle({backgroundColor: \'#FFF\', color: \'#000\'}).value = \'background_color\'; Control.ColorPicker.activeColorPicker.close();">background color</button></div>' +
          '<div><button onclick="Control.ColorPicker.activeColorPicker.field.setStyle({backgroundColor: \'#FFF\', color: \'#000\'}).value = \'custom_color\'; Control.ColorPicker.activeColorPicker.close();">custom color</button></div>' +
          '</div>';
  ...
  update : function(x, y) {
    ...
    var value = YAHOO.util.Color.rgb2hex(rgb[0], rgb[1], rgb[2]);
