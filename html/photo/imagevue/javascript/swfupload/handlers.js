...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("您的队列文件过多.\n" + (message === 0 ? "您已达到上传限制." : "你可以选择 " + (message > 1 ? "上传 " + message + " 个文件." : "1个文件.")));
			return;
		}
		...
		case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
			...
			this.debug("错误代码: 文件太大了, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		...
		case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
			...
			this.debug("错误代码: 无效文件类型, 文件名: " + file.name + ", 文件大小: " + file.size + ", 信息: " + message);
			break;
		default:
			if (file !== null) {
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.IO_ERROR:
			...
			this.debug("错误代码: 服务器错误, 文件名: " + file.name + ", 信息: " + message);
			break;
