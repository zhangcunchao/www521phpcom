...
class SEO_RankChecker
{
	...
	public function getIndexedBaidu()
	{
		...
		preg_match('/找到相关结果数约([0-9\,]+)个/si', $data, $p);
...
//获取yahoo的收录数量
	...
	*/
	public function getPagerank()
	{
		...
		$url="http://www.aizhan.com/ajaxAction/pr.php?domain=$this->url&prcheck=1";
		$data = $this->getContent($url);
		preg_match('|<img src="http:\/\/static.aizhan.com\/images\/pr\/pr(.*?)\.gif" align="absmiddle" \/>|i', $data, $p);
	...
	*获取百度权重
	*/
	function getBaidurank($s){
		$head = "Mozilla/5.0 (Windows NT 5.1; WOW64; rv:9.0.1) Gecko/20120423 Firefox/9.0.1";
		$HTTP_Server="www.aizhan.com"; 
		$HTTP_URL="/baidu/".$s; 
		$ch = curl_init(); 
	    ...
	    $res = curl_exec($ch); 
		preg_match('/<img align\="absmiddle" src="http\:\/\/static\.aizhan\.com\/images\/brs\/(.*?)\.gif">/i',$res,$count);
		curl_close ($ch); 
		return $count[1];
	}
